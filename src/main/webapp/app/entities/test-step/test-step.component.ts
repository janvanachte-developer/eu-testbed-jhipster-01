...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/test-step'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/test-step', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: TestStep) {
        return item.id;
    }
    registerChangeInTestSteps() {
        this.eventSubscriber = this.eventManager.subscribe('testStepListModification', (response) => this.loadAll());
    }
