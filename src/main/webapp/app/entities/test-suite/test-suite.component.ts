...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/test-suite'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: TestSuite) {
        return item.id;
    }
    registerChangeInTestSuites() {
        this.eventSubscriber = this.eventManager.subscribe('testSuiteListModification', (response) => this.loadAll());
    }
